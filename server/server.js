...
// --- REVERSE PROXY & TEK ÇATI AYARLARI ---
// Node.js hem API'yi hem de React sitesini sunar.

// 1. Statik Dosyalar (dist klasörü)
// Sunucudaki yapı: C:/DeryaGrupWebApi (Server) ve C:/inetpub/wwwroot/.../dist (Site)
// Eğer 'dist' klasörü 'server'ın bir üstündeyse:
app.use(express.static(path.join(__dirname, '../dist')));

// Eğer 'dist' klasörü 'server'ın yanındaysa (Sizin DeryaGrupWebApi klasöründe):
// app.use(express.static(path.join(__dirname, 'dist'))); 

// 2. React Router (Sayfa Yenileme Desteği)
app.get('*', (req, res) => {
    // API isteği ise ve bulunamadıysa 404 dön
    if (req.url.startsWith('/api') || req.url.startsWith('/uploads')) {
        // Fallback: Belki dosya vardır?
        return res.status(404).json({ message: 'Not Found' });
    }
    // Değilse index.html gönder (React başlasın)
